...
* iOS: Bluetooth permission dialog now appears when requested instead of when the app is initialized.
  Note: Requesting Bluetooth status will also prompt the permission dialog (see issue [#591](https://github.com/Baseflow/pagando-permission-handler/issues/591)).
...
* Solved bug where output would log that there is no permission in the manifest for BLUETOOTH_SCAN, BLUETOOTH_ADVERTISE, and BLUETOOTH_CONNECT on Android 11 devices and lower(see issue [#691](https://github.com/Baseflow/pagando-permission-handler/issues/691)).

## 8.2.3

* iOS: Enhanced the `bluetooth` permission for iOS 13 and up, so the user gets prompted with the "Bluetooth" permission dialog (see issue [#591](https://github.com/Baseflow/pagando-permission-handler/issues/591)).
...
* iOS: fixed memory error that occurred on iOS 12.2 and below (see issue [#638](https://github.com/Baseflow/pagando-permission-handler/issues/638)).
...
* Suppress deprecation warnings on iOS for code that specifically targets older iOS versions (see issue [#607](https://github.com/Baseflow/pagando-permission-handler/issues/607)).

## 8.1.1

* Fixed deprecation issue when checking phone capabilities on iOS (see issue [#597](https://github.com/Baseflow/pagando-permission-handler/issues/597)).
...
* Workaround for ignoring battery optimizations on pre-M Android devices (see PR [#376](https://github.com/Baseflow/pagando-permission-handler/pull/376)).
...
* Removed PermissionStatus.undetermined, PermissionStatus.denied will be used instead (see our "[Changes in 6.0.0](https://github.com/Baseflow/pagando-permission-handler/wiki/Changes-in-6.0.0)" wiki page for more details).
...
* Revert pull-request [#317](https://github.com/Baseflow/pagando-permission-handler/pull/317)

## 5.0.0+hotfix.9

* Solved an issue where kCLAuthorizationStatusAuthorizedWhenInUse was returning PermissionStatusDenied (see [#317](https://github.com/Baseflow/pagando-permission-handler/pull/317))

## 5.0.0+hotfix.8

* Solved an issue on iOS where requesting notification permissions returned prematurely (see pull-request [#297](https://github.com/Baseflow/pagando-permission-handler/pull/297))
...
* Solved an issue on iOS where requesting notification permissions always resulted in a "granted" result (see pull-request [#289](https://github.com/Baseflow/pagando-permission-handler/pull/289))

## 5.0.0+hotfix.5

* Remove the use of the deprecated pre-iOS 8 API causing users to compile issues (see issue [#277](https://github.com/Baseflow/pagando-permission-handler/issues/277)).

## 5.0.0+hotfix.4

* Solve issue on Android causing an IllegalStateException ([#267](https://github.com/Baseflow/pagando-permission-handler/issues/267)).
...
* **BREAKING**: Implemented more intuitive API exposed by `permission_handler_platform_interface: 2.0.0` ([#230](https://github.com/Baseflow/pagando-permission-handler/issues/230)).

## 4.4.0+hotfix.4

* Android: solved issue where `permission_handler` is used in a service (see [#251](https://github.com/Baseflow/pagando-permission-handler/issues/251))
